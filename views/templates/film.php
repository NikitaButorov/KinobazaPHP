<?php
if (!function_exists('displayMoviesFromDatabase')) {
function displayMoviesFromDatabase()
{
    ...
    foreach ($movies as $movie) {
        ...
        if ($result->num_rows > 0) {
            ...
            echo "<a href='/KINOBAZAPHP/views/{$row['title']}.php'><img src='{$row['image_address']}' alt='{$row['title']}'></a>";
            echo "<div class='film_label'><a href='/KINOBAZAPHP/html/views.php'>{$row['title']}</a></div>";
            echo "</div>";
        }
    }
...
if (!function_exists('displaySerialsFromDatabase')) {
